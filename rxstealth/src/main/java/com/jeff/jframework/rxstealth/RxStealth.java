 ...
 *
 * @author Jeff
 * @date 2019/11/29 17:50
 *
 * <a href="mailto:feijeff0486@gmail.com">Contact me</a>
 * <a href="https://github.com/feijeff0486">Follow me</a>
 */
public class RxStealth extends AbstractStealth<RxStealthFragment> {
    private static final String TAG = "RxStealth";
    static final Object TRIGGER = new Object();

    public RxStealth(@NonNull Activity activity) {
        super(activity);
    }

    public RxStealth(@NonNull Fragment fragment) {
        super(fragment);
    }
    ...
    @Override
    protected RxStealthFragment createFragment() {
        return new RxStealthFragment();
    }
                ...
                return request(o, permissions)
                        ...
                        .flatMap(new Function<List<Permission>, ObservableSource<Permission>>() {
                            ...
                            public ObservableSource<Permission> apply(List<Permission> permissions) {
                                ...
                                return Observable.just(PermissionHelper.combine(permissions));
                            }
                        });
            }
        };
    ...
    private Observable<Permission> request(final Observable<?> trigger, final String... permissions) {
        if (permissions == null || permissions.length == 0) {
            throw new IllegalArgumentException("RxHelper.request/requestEach requires at least one input permission");
        }
    ...
    private Observable<?> pending(final String... permissions) {
        for (String p : permissions) {
            if (!getFragment().containsByPermission(p)) {
                return Observable.empty();
            }
        }
    ...
    private Observable<Permission> requestImplementation(final String... permissions) {
        ...
        for (String permission : permissions) {
            getFragment().log("Requesting permission " + permission);
            ...
            PublishSubject<Permission> subject = getFragment().getSubjectByPermission(permission);
            // Create a new subject if not exists
            if (subject == null) {
                unrequestedPermissions.add(permission);
                subject = PublishSubject.create();
                getFragment().setSubjectForPermission(permission, subject);
            }
    ...
    void requestPermissionsFromFragment(String[] permissions) {
        getFragment().log("requestPermissionsFromFragment " + TextUtils.join(", ", permissions));
        getFragment().requestPermissions(permissions);
    }
