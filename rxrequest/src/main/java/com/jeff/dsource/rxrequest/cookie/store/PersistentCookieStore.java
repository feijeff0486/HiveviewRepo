package com.jeff.dsource.rxrequest.cookie.store;

import android.text.TextUtils;
import android.util.Log;

import com.jeff.jframework.tools.preference.IPreference;
...
public class PersistentCookieStore implements CookieStore {
    ...
    private static final String COOKIE_PREFS = "rxrequest_cookie";        //cookie使用prefs保存
    ...
    private IPreference cookiePrefs;

    public PersistentCookieStore() {
        cookiePrefs= PreferencesUtils.getPreference(COOKIE_PREFS);
        cookies = new HashMap<>();
    ...
    private void saveCookie(HttpUrl url, Cookie cookie, String name) {
        ...
        //文件缓存
        cookiePrefs.put(url.host(), TextUtils.join(",", cookies.get(url.host()).keySet()));
        ...
        if (cookies.containsKey(url.host())) {
            ...
            for (String cookieName : cookieNames) {
                if (cookiePrefs.contains(COOKIE_NAME_PREFIX + cookieName)) {
                    cookiePrefs.remove(COOKIE_NAME_PREFIX + cookieName);
                }
            }
            cookiePrefs.remove(url.host());
            //内存移除
